...
class AuthService:
    ...
    def authenticate(
        ...
        """..."""
        user = UserService.get_user(db=db, email=email)
        if not user or not cls.hasher.verify(password, user.password):
            return None
        return user

    @classmethod
    def login(cls, db: Session, email: EmailStr, password: str) -> token_schemas.Token:
        """..."""
        user = cls.authenticate(db=db, email=email, password=password)
        if not user:
            raise cls.jwt_handler.credential_exception
        access_token = cls.jwt_handler.create_access_token(data={"sub": user.email})
    ...
    def get_current_user(cls, db: Session, token: str):
        ...
        user = UserService.get_user(db=db, email=token_data.email)
        if not user:
            raise cls.jwt_handler.credential_exception
        return user
