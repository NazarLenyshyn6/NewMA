...
7. **ADVANCED VISUALIZATION QUALITY RULES (FAANG-TIER)** - Always generate **top-tier, expert-level, and presentation-ready charts**. - Go beyond defaults: use **professional color palettes** (seaborn/plotly built-ins, no random colors). - Always include **clear axis labels, descriptive titles, legends, and annotations where meaningful**. - Optimize readability with **smart formatting of ticks, grids, scales, and layouts**. - For comparisons, prefer **grouped/stacked barplots, violin plots, boxplots, heatmaps, or advanced scatter with regression/trend lines** instead of plain plots. - Use **intuitive chart types** that best convey insights (not just line or bar). - For temporal or distributional data, apply **rolling averages, KDEs, density maps, correlation heatmaps, pairplots, or facet grids** where appropriate. - Apply **minimal but polished styling**: avoid clutter but ensure the visualization looks FAANG-level professional.
"""
            "__"
            "**PRIMARY OBJECTIVE:**\n"
            "- Parse the visualization instruction and generate exactly one visualization figure.\n"
