...
"""
            ...
            "- NEVER include any visualization unless explicitly allowed (see Visualization Rule below).\n"
            ...
            "**VISUALIZATION RULE (EXTREMELY STRICT, ADVANCED, USER-FRIENDLY):**"
            "- You must write code for visualization **only if** the instruction explicitly requests it."
            "- Generate **exactly one plot** per instruction — no more, no less."
            "- The plot must be visually polished, clear, and fully interpretable **without any additional explanation**."
            "- Include titles, axis labels, legends, color encoding, and annotations as needed to make it self-explanatory and insightful."
            "- Use appropriate chart types (histogram, bar, scatter, line, box, heatmap, etc.) depending on the data and context."
            "- Ensure readable font sizes, proper layout, and color schemes that are user-friendly and distinguishable."
            "- Incorporate advanced visual features if meaningful: e.g., trend lines, distributions, highlights, outliers, or annotated metrics."
            "- All generated code for the plot must be **self-contained** and store the resulting image as a **Base64 string** in the global `images` list."
            "- Initialize the images list at the beginning: `images = []`."
            "- After execution, `images` must contain only one Base64-encoded string corresponding to this single plot."
            "- NO other plots, text output, or additional explanation are allowed."
            "- Visualization must reflect the data faithfully, highlighting key patterns, trends, or anomalies for immediate understanding by the user."
        ...
        HumanMessagePromptTemplate.from_template(
            ...
            "- Visualization code must generate **only one plot**, store it as Base64 in `images`, and nothing else."
        ),
    ]
)
